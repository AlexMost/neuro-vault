...
How a vault owner's `for-external-agents.md` reaches an agent: the delivery channel, its size budget, and what happens when the file is missing or oversized.
...
`src/lib/obsidian/vault-conventions.ts` owns the file: `CONVENTIONS_PATH` (the vault-relative location), `readVaultConventions(vaultPath)` (the best-effort read), `CONVENTIONS_CHAR_CAP` (the response-side size cap) and `capConventions(raw)` (the bounded slice). Every vault entry exposes the reader as `IVaultEntry.readConventions()`, built by `conventionsReaderFactory` in `IVaultEntryDeps` like every other per-entry dependency. One reader, so every consumer agrees by construction on where the file lives, what counts as empty, and what a failed read means.

## Why it exists

The content is delivered over exactly one channel.

**The overview channel.** `computeVaultOverview` puts the text in a `conventions` field, which both surfaces inherit: the `get_vault_overview` tool response and the `vault://overview` resource. Tool responses arrive intact in every client we have measured, sub-agents included. (Claude Code does apply a much larger, configurable cap to tool output.)

The MCP `instructions` string carries a **pointer** to that channel and never a copy of the file. It once carried both: one conventions block per vault, composed at startup, ahead of the server preamble. Measurement showed Claude Code applies its 2048-character `instructions` cap *per server, not per vault*, so with several vaults the first consumed the budget and later blocks arrived as nothing — and because conventions led, any file over roughly 1,316 characters deleted the preamble instead. Silently, since `instructions` has no response shape to carry a truncation flag. [ADR-0012](../adr/0012-conventions-leave-the-instructions-channel.md) records the measurements and the decision to cut the channel rather than patch it.
...
### The pointer in `instructions`

`SERVER_INSTRUCTIONS` is a module constant in `src/server.ts` — no registry parameter, no file reads, identical for every configuration. It is the 693-character server preamble plus a ~240-character paragraph naming `get_vault_overview`, stating that a vault's conventions arrive on that response rather than in `instructions`, and saying to call it before reading or writing notes. 936 characters total, 54% under the client's 2048-character cap, and nothing a vault owner does can move it.

`test/server-instructions.test.ts` guards that invariant: the composed string is a constant beneath the cap, contains no vault-file content when the server starts against a vault that has one, and still names `get_vault_overview`. The suite it replaced pinned a 1,227-character conventions fixture and varied only the preamble — so the free variable in production was the constant in CI, which is how the broken delivery shipped.

### Size cap and the truncation flag

`CONVENTIONS_CHAR_CAP` is **8,000 characters**, the overview channel's own cap. When a file exceeds the cap, `capConventions` returns a bounded slice ending in a single `…` marker and the payload sets `conventions_truncated: true`. When the file fits, `conventions_truncated` is absent, so a consumer reading `conventions` without the flag can treat it as the complete file. Truncation is never silent.
...
Each vault's conventions travel with that vault:
...
No extra wiring was needed for either: the field is produced at the compute layer, and both the fan-out and the per-vault resource registration already iterate registry entries. No vault's delivery depends on how many vaults precede it — the property the `instructions` channel could not offer, since its cap is per server.

### Failure is always absence

Reading the file is best-effort. A missing, unreadable, or permission-denied `for-external-agents.md` is treated as absent: `readVaultConventions` swallows the error and returns `null`, and `computeVaultOverview` additionally guards the call itself so that even a rejecting injected reader costs the caller nothing. `get_vault_overview` still returns its full structural snapshot with no `conventions` key and reports no error, and in fan-out the vault appears in `results_by_vault` — **not** in `failed_vaults`. Server startup cannot be affected at all: it never consults the file. An optional file must never be able to turn a working call into an error.
...
- `CONVENTIONS_CHAR_CAP` belongs to the overview channel and is not an `instructions` budget — `instructions` carries no conventions to size ([ADR-0012](../adr/0012-conventions-leave-the-instructions-channel.md)).
...
- `src/server.ts` — `SERVER_INSTRUCTIONS`, the constant carrying the pointer.
- `test/server-instructions.test.ts` — the constant-under-the-cap guard and the no-vault-content assertion.
