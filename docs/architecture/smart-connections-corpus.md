...
- An empty corpus throws inside the loader. Its caller — `VaultRegistry.create` — catches that throw and records `semanticAvailable: false` on the affected `VaultEntry`. Startup no longer fails on a single empty corpus; the failure surfaces at semantic-tool-call time as `SEMANTIC_INDEX_NOT_FOUND` for the explicit-vault-only tools that require a semantic index (`get_similar_notes`, `find_duplicates`, via `resolveSemanticVault`). `search_notes` — the only semantic-capable fan-out tool — does not skip such a vault: it still runs and falls back to lexical-only matches for it, fused into the same ranked list (see [`fan-out.md`](./fan-out.md#skipped-vs-failed) and [`rank-fusion.md`](./rank-fusion.md#degradation-modes)). The invariant _inside_ the loader is unchanged — it still throws on an empty result; only the caller semantics changed.
