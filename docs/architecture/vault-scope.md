...
**Also governed — the server's own embedding corpus.** The indexer takes its path set from the scoped scan and applies no exclusion rule of its own, so a note the scope hides is never embedded and a shard left behind by a scope change is deleted on the next reconcile — see [`own-corpus.md`](./own-corpus.md). Embedding-side rules decide only *what text of an included note* is embedded (the `min_chars` gate, the character truncation), never which notes are included.

**Still ungoverned — what serves semantic results today.** Everything that reads `entry.corpus` (the Smart Connections AJSON) is outside this list: the semantic leg of `search_notes`, `get_similar_notes`, and `find_duplicates`. Those tools still serve from the plugin's corpus, whose membership is whatever its own `file_exclusions` config produced when it wrote the vectors — so a `Templates/` note the plugin happened to embed still comes back under `semantic_matches` while being absent from `lexical_matches`, tag counts, and `total_notes`. The own corpus exists but nothing reads it yet; the `own-backend-integration` slice switches the tools over and closes that gap.
