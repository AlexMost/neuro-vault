...
| `normalizeNotePath(raw)` (in `src/lib/obsidian/note-path.ts`) | Every tool that takes a `path` for an _individual note_ — `create_note`, `edit_note`, `set_property`, `read_property`, `remove_property`, `get_note_links`, `get_similar_notes` | Wraps `normalizeVaultPath` then auto-appends `.md` when the final segment has no extension.                                                                     |
