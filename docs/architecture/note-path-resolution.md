...
| `normalizeNotePath(raw)` (in `src/lib/obsidian/note-path.ts`) | Every tool that takes a `path` for an *individual note* — `create_note`, `edit_note`, `set_property`, `read_property`, `remove_property`, `get_note_links` | Wraps `normalizeVaultPath` then auto-appends `.md` when the final segment has no extension. |
...
## What `normalizeNotePath` does *not* do
...
- It does not strip Markdown formatting from extensions — `trim()` is applied upstream by `normalizeVaultPath`.
