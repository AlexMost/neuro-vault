...
- `operations/` — direct vault operations — 12 tools, grouped as note body (`read_notes`, `create_note`, `edit_note`, `read_daily`, `append_daily`), structured queries (`query_notes`), frontmatter properties (`set_property`, `read_property`, `remove_property`, `list_properties`), and tags (`list_tags`, `get_tag`)
