...
- `src/lib/vault-registry.ts` builds one entry per configured vault from the factories in `buildDefaultVaultEntryDeps` (`src/server.ts`), constructing `FsVaultReader`, `FsVaultWriter`, `WikilinkGraphIndex` and the Smart Connections corpus index per vault. Both modules then read those entries off the registry rather than constructing anything themselves.
