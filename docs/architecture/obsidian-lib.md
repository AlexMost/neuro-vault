...
- **`vault-reader.ts`** — `VaultReader` interface and `FsVaultReader` (filesystem-backed implementation). Reads notes, splits frontmatter, supports subtree scanning filtered through an optional `VaultScope`. Reports stale-path conditions via `ScanPathNotFoundError`. See [`./vault-reader.md`](./vault-reader.md).
- **`vault-scope.ts`** — `VaultScope`, `createVaultScope`, and `gitignoreLinesToPatterns`: the per-vault definition of which files are discoverable (dot-path/`Templates`/`.gitignore`/config exclusion layers), exposing both a fast-glob `ignorePatterns` prune and an authoritative `isExcluded` predicate. See [`./vault-scope.md`](./vault-scope.md).
- **`vault-scope-config.ts`** — `loadVaultScope(vaultRoot)`, which reads a vault's root `.gitignore` and `.neuro-vault/config.json` and builds its `VaultScope`; never throws — an unreadable or invalid config falls back to defaults with a stderr warning. See [`./vault-scope.md`](./vault-scope.md).
...
- **`vault-conventions.ts`** — the optional owner-authored `.neuro-vault/for-external-agents.md`: `CONVENTIONS_PATH`, the best-effort `readVaultConventions(vaultPath)` (missing / empty / unreadable all collapse to `null`), and `capConventions` with `CONVENTIONS_CHAR_CAP`. Consumed by the `get_vault_overview` payload and the `vault://overview` resource — the only channel that carries conventions. See [`./vault-conventions.md`](./vault-conventions.md).
...
- `src/lib/vault-registry.ts` builds one entry per configured vault from the factories in `buildDefaultVaultEntryDeps` (`src/server.ts`): `loadVaultScope` first (so `FsVaultReader` can be constructed with the resulting `VaultScope`), then `FsVaultWriter`, `WikilinkGraphIndex`, the per-vault conventions reader (`readVaultConventions` bound to that vault's path), and the Smart Connections corpus index per vault. Both modules then read those entries off the registry rather than constructing anything themselves.
