...
| "What's the status of Quarterly review?"     | `search_notes({ query: "Quarterly review" })` to resolve the path, then `read_notes({ paths: ["..."], fields: ["frontmatter"] })` and read the `status` key |
