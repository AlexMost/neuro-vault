...
- Use **structural tools** first — your assistant's own file/path/title tools, or vault operations like `read_notes`, `read_property`, `get_tag` — for exact file, title, path, daily note, tag, property, wikilink, backlink, and link-traversal requests.
...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| "Read the note 'Q1 OKRs'"                    | `search_notes({ query: "Q1 OKRs" })` to resolve the path, then `read_notes({ paths: ["..."] })` |
