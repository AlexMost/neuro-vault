...
2. **Otherwise.** Use the same scan+match pipeline as `runQueryNotes`, but **without its 1000-result cap and sort step** (we need every matching path; ordering is irrelevant). Concretely: extract the existing batched scan+sift loop from `runQueryNotes` into a private helper `collectMatchingPaths(filter, deps): Promise<string[]>` that returns the unbounded match set. Both `runQueryNotes` (which then applies its sort + `limit` + `truncated` logic) and `listMatchingPaths` (which wraps the array in a `Set`) call it. The compiled internal filter passed to the helper is `$and([...path_prefix-clause, { tags: { $in: filter.tags } }, ...filter.frontmatter])` with absent fields omitted.
...
| File                                                     | Change                                                                                                                                                                                                                                                                             |
| -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/modules/operations/query/index.ts` (or sibling)     | Extract `collectMatchingPaths(filter, deps)` from `runQueryNotes` (uncapped, unsorted). Export `listMatchingPaths(filter, deps)` (path_prefix-only fast-path + delegate). Refactor `runQueryNotes` to call `collectMatchingPaths` then apply sort + limit + truncated.             |
