...
An MCP server has three ways to put text in front of an agent: the `instructions` string returned at `initialize`, each tool's `description`, and each tool's response. We had been treating `instructions` as the general-purpose channel, and it had grown to a 10,803-character preamble with fourteen headings, plus a separate 492-character overview hint — much of it restating what tool descriptions already said.
...
- The `instructions` preamble is now 693 characters and holds only cross-cutting guidance no single tool owns: the vault's role as a second brain, the operations-vs-semantic routing rule, and the project-scope discovery order. The per-tool sections and the multi-vault section are gone, because each multi-vault-aware tool's own description already states the contract that is true of *that* tool. The five fan-out tools (`search_notes`, `query_notes`, `get_vault_overview`, `list_tags`, `list_properties`) describe the `results_by_vault` / `failed_vaults` shape; the nine that cannot fan out (`read_notes`, `create_note`, `edit_note`, `read_daily`, `set_property`, `remove_property`, `get_note_links`, `find_duplicates`, `get_similar_notes`) state the opposite contract through `EXPLICIT_VAULT_SUFFIX` — omitting `vault` returns `VAULT_REQUIRED`. `describeMultiVault` (`src/lib/vault-param.ts`) is only the shared wrapper: it prefixes the registered vault names and concatenates whichever suffix the call site passes. `search_notes` composes its multi-vault text inline instead of going through it. That no single sentence covers all fourteen is the point — the contract rides on the description of the tool it applies to.
- Vault conventions now travel primarily on the `get_vault_overview` response, where the client does not apply the `instructions` cap, and reach sub-agents. See [`docs/architecture/vault-conventions.md`](../architecture/vault-conventions.md).
