...
- **Choice**: default merged cap: quick → 5, deep → 12; `limit` overrides it in every mode. Internal candidate-pool caps are unchanged (semantic 3/8, lexical 5/10, expansion 3/seed, block caps as today). `truncated` is always present (both string and array queries) and true when the merged cap dropped candidates.
