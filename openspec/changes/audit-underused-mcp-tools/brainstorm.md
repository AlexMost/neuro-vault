...
fortnight and still be the *only* way to do something. So the deliverable is an **audit**, not a
...
Source: reports `{ totalNotes, totalBlocks, embeddingDimension, modelKey }` from the *embedding
corpus*. Live check: `get_stats` → `704 notes / 16,595 blocks / dim 384 / bge-micro-v2`, while
...
*keep*. The user was shown this and chose to **remove anyway** — a deliberate surface cut,
...
(`blocked_by`, `excalidraw-plugin`, and four `count: 0` keys) is dropped. So it is *not* a strict
...
*recommends* it, so removing it has a doc dependency. The user confirmed **remove**.
...
3. **Scrub references** to the three removed tools from all *live* docs (guides, architecture,
