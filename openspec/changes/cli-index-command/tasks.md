...
- [ ] 1.1 Failing test → implementation: restructure `parseConfig` around
  ...
  ship together.
- [ ] 1.2 Failing test → implementation: `index --vault` reuses
...
- [ ] 1.3 Test: every existing `cli-startup-flags` scenario still holds under
...
- [ ] 2.1 Failing test → implementation: `runIndexCommand(options, deps)` in a
...
- [ ] 2.2 Failing test → implementation: progress rendering — TTY: one
...
- [ ] 2.3 Failing test → implementation: per-vault summary line printed on
  completion with all six `ReconcileSummary` counts, and warnings left on
  stderr.
- [ ] 2.4 Failing test → implementation: exit-code contract — 0 only when
...
- [ ] 2.5 Test: the runner constructs no MCP server and connects no transport
...
- [ ] 3.1 Outside the test suite: run `npm run dev -- index --vault <real vault>`
...
- [ ] 4.1 `npm test`, `npm run lint`, `npm run typecheck`,
  `npx openspec validate --all` all green; paste the output in the PR body.
- [ ] 4.2 Confirm the slice stayed thin: no MCP tool contract change, no
