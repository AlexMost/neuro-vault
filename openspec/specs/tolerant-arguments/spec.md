...
How the MCP tool-input boundary tolerates near-miss arguments instead of dead-ending: declared key aliases are renamed to their canonical parameter, stringified JSON collections are parsed when unambiguous, and an unrecoverable value fails with a message that names the expected shape — while genuinely unknown keys are still rejected. Keeps an agent that guesses the contract from silently failing without a pivot.
